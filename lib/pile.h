...
class Pile {
 ...
 public:
  ...
  std::optional<solitaire::Run> peek(
      unsigned int);                    // take a look at the top N cards
  std::optional<solitaire::Run> pop();  // remove the top card from the top run
  std::optional<solitaire::Run> take(
      unsigned int);  // take the top N visible cards (if they are movable)
  void put(
      std::vector<Card>);  // add a stack of viable cards to the top of the pile
  void put(
      solitaire::Run);  // add a stack of viable cards to the top of the pile
