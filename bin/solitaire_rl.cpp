/* solitaire_rl.cpp
 * for starters, just a copy of solitaire.cpp but using the gnu readline library
...
string collapse_internal_spaces(const string &s) {
...
int play(mt19937 generator) {
  ...
  while ((rlbuf = readline("cmd: ")) != nullptr && is_not_exit(rlbuf)) {
    ...
    if (is_help(cmd)) {
      ...
      try {
        string movecmd_str = remove_m_prefix(collapse_internal_spaces(cmd));
        MoveCmd m_cmd = MoveCmd(movecmd_str);
        b.move(m_cmd);
